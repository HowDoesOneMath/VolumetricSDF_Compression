...
void VSMC_Compressor::SetTimeLogFile(std::string time_log_name)
{
    ...
    tl.CreateNewLogger(buffer_writing_time_logger_name, new TimeLogger::IndependentLogger(""));
    ...
    tl.CreateNewLogger(buffer_reading_time_logger_name, new TimeLogger::IndependentLogger(""));
...
#endif
{
    ...
    tl.GetLogger(buffer_writing_time_logger_name)->StartTimer();
#endif

    sfb.WriteVectorToBuffer(compression_buffer);

#if VSMC_TIME_LOGGING
    tl.GetLogger(buffer_writing_time_logger_name)->MarkTime();
...
#if USE_DOUBLE_DISPLACEMENTS
std::shared_ptr<cimg_library::CImg<double>> VSMC_Compressor::CompressIntraWithoutTexturing(
    VV_Mesh& input_mesh, VV_SaveFileBuffer& sfb)
#else
std::shared_ptr<cimg_library::CImg<unsigned char>> VSMC_Compressor::CompressIntraWithoutTexturing(
    VV_Mesh& input_mesh, VV_SaveFileBuffer& sfb)
#endif
{
#if USE_DOUBLE_DISPLACEMENTS
    ...
    to_return->assign(disp_width, disp_width, 1, 3, 0);
    dp.FillImageBlocksRaster(*to_return, b_size, *displacements, 0, displacement_max, min_data_val, max_data_val);
...
#if VSMC_TIME_LOGGING
    tl.GetLogger(buffer_reading_time_logger_name)->StartTimer();
#endif
...
#if VSMC_TIME_LOGGING
    tl.GetLogger(buffer_reading_time_logger_name)->MarkTime();
    tl.GetLogger(draco_decompression_time_logger_name)->StartTimer();
#endif
    ...
    std::string output_texture_tag, std::string displacement_texture_tag, size_t starting_frame, size_t ending_frame, size_t digits_per_number)
{
    ...
    tl.GetLogger(buffer_writing_time_logger_name)->ResetTotalTime();
    ...
    tl.PrintEmptyLine();
    tl.PrintTotalAndAverageAndGreatestTime(buffer_writing_time_logger_name, "Total time", "Average time", "Greatest time", " Buffer Writing: ");
    tl.PrintEmptyLine();
...
bool VSMC_Compressor::CompressSequenceWithoutTexturing(std::string root_folder, SequenceFinderDetails mesh_sf, std::string output_file_name, std::string displacement_texture_tag, size_t starting_frame, size_t ending_frame, size_t digits_per_number)
{
    if (!initialized)
    {
        std::cout << "INITIALIZE BEFORE USING" << std::endl;
    ...
    std::vector<SequenceFinderDetails> details = { mesh_sf };
    ...
    for (size_t i = starting_frame; i < item_count; ++i, ++relative_ind)
    {
        ...
        auto imgs = CompressIntraWithoutTexturing(to_compress_mesh, sfb);
...
#endif
        disp_file_name = displacement_texture_tag + "_" + GetNumberFixedLength(i, digits_per_number) + ".jpg";
        imgs->save_jpeg(disp_file_name.c_str(), jpg_q);
